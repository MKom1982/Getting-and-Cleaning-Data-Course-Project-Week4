Description of the tidy data set "Xtidy.txt"

Variables in the tidy data set
Tidy data contains 180 rows and 82 columns.
Data are divided according to subject and activity type.
There were 30 subjects (numbered 1:30) and 6 possible activity types (with following names: WALKING, WALKING_UPSTAIRS, WALKING_DOWNSTAIRS, SITTING, STANDING, LAYING), which in result gives 180 possibilities (=rows in data set).
Each row has averaged variables for each subject and each activity, as well as information about subjects and activity types.
Set contains only the variables with mean and standard deviation values of each measurement.

Names of the variables:
    mean(): Mean value
    std(): Standard deviation

Tidy data contains 82 columns (subject label, activity type, activity label and 79 variables).
Columns name are as follows:

 [1] "subject_id"                      "activ_type"                     
 [3] "activ_id"                        "tBodyAcc-mean()-X"              
